...
int main(int argc, char* argv[])
{
    ...
    for (int i=0; i<mRecConnectionTable.getSize(); i++)
    {
        ...
        if (face_minus_id == 1){ // Front-Back connect
        ...
        else if (face_minus_id == 5){ // Top-Bottom connect
    ...
    if (method==Method::SIPG){
        const double rel_skew = NumUtilities::SkewRelFrobenius(A);
        if (!NumUtilities::IsSymmetric(A, 1e-12)) {
            std::cerr << "[sym] Global A not symmetric: rel_skew=" << rel_skew << "\n";
        }
    }
...
if ((A * u_h).isApprox(b) == false) {
    ...
    std::cout << "Solver method=" <<  "Sparse Direct" << ", package=" << "Eigen intel MKL PardisoLDLT+PardisoLU" << "\n";
    ...
    if (write_h5=="w" || write_h5=="W") {
        ...
        grid.createDataSet("A", Eigen::MatrixXd(A));
