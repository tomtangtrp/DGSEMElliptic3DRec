...
int main(int argc, char* argv[])
{
    ...
    //----------------- Eigen built in sparse direct solver--------------------
    /*
    ...
    */
    ...
    if (method == Method::SIPG){
        ...
        // Eigen::ConjugateGradient<SparseMatrix, Eigen::Lower|Eigen::Upper, Eigen::DiagonalPreconditioner<double> > cg;
        Eigen::ConjugateGradient<SpMat, Eigen::Lower|Eigen::Upper, Eigen::IncompleteLUT<double>> cg;
        cg.setMaxIterations(4000);
        ...
        solver.setTolerance(1e-10); // Higher precision usually reqreuis good pre-conditioning 
        solver.compute(A);
        u_h = solver.solve(b);
...
if ((A * u_h).isApprox(b) == false) {
    ...
    std::cout << "Solver method=" <<  "Sparse Direct" << ", package=" << "Eigen Sparse Iterative Non-Matrix-Free" << "\n";
